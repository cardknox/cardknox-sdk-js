...
export default class SignatureReader {
    ...
    async _getSignature() {     // prefixed as private to prevent usage as this is only part of the process
        const getSignatureResponse = await this.ipDeviceCommunicator.getData(GET_SIGNATURE_HASH);

        const getDeviceResponseReader = getSignatureResponse.body.getReader();
        let { value: chunk } = await getDeviceResponseReader.read();
    ...
    async doSignature() {
        const doSignatureResponse = await this.ipDeviceCommunicator.getData(DO_SIGNATURE_HASH);

        const doGeviceResponseReader = doSignatureResponse.body.getReader();
        let { value: chunk } = await doGeviceResponseReader.read();
